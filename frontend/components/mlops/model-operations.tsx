...
export function ModelOperations() {
  ...
  const evaluateDrift = async () => {
    ...
    try {
      if (isLive) {
        const activeModelVersion = championVersion || liveProjection?.version;
        if (!activeModelVersion) throw new Error("No verified Champion model is available for live drift evaluation.");
    ...
    <div className="mt-6 space-y-6">
      ...
      <section className="overflow-hidden rounded-xl border border-gov-primary/20 bg-gov-primary text-white shadow-card">
        <div className="compass-grid-overlay grid gap-5 px-5 py-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)] lg:px-6">
          ...
          <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3"><HeroMetric icon={Target} label="Candidate accuracy" value={evidenceLoading && isLive ? "Loading" : candidateDisplay ? `${Math.round(candidateDisplay.accuracy * 100)}%` : isLive ? "Unavailable" : "Run training"} /><HeroMetric icon={Scale} label="Macro F1" value={evidenceLoading && isLive ? "Loading" : candidateDisplay ? candidateDisplay.macroF1.toFixed(2) : isLive ? "Unavailable" : "Run training"} /><HeroMetric icon={UserCheck} label={isLive ? "Training evidence" : "Review threshold"} value={isLive ? liveTrainingState : "62%"} /></div>
        </div>
      </section>
