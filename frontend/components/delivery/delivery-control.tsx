...
const GATES: Gate[] = [
  ...
  { id: "sast", label: "SAST and dependency", detail: "Run Bandit, audit locked dependencies, scan secrets, and emit the compass-sbom.cdx.json CycloneDX artifact.", command: "bandit -r src scripts -ll -ii && pip-audit --no-deps", evidence: ".github/workflows/devsecops.yml", state: "pass", icon: ScanSearch },
  { id: "iac", label: "IaC and STIG policy", detail: "Validate Terraform and SAM, then evaluate the demonstrable STIG control profile with CloudFormation Guard.", command: "terraform fmt -check -recursive && terraform validate && sam validate --lint && cfn-guard validate", evidence: "security/policies/compass.guard", state: "pass", icon: FileCode2 },
